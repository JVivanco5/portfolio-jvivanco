      ...
      {dataCounter.map(({ id, endCounter, text, lineRight, lineRightMobile }) => (
        <div key={id} className={`${lineRight && 'ltr'}`}>
          <div className={`${lineRight && 'px-4 border-2 border-transparent md:border-e-gray-100'}
            ${lineRightMobile && 'md:border-e-gray-100'}
          `}>
            <p className="flex mb-2 text-2xl font-extabold md:text-4xl text-secondary"><CountUp end={endCounter} start={0} duration={5} /></p>
